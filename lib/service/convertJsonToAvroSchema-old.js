...
const exempleNamespace = "com.group.generatedAvroSchema"
/*
 * @param {any} json
...
module.exports = function convert(json, options) {
    ...
    if (output.type === 'array') {
        processOutput = processArray(json, options, exempleNamespace)
        output.type = processOutput.type
...
function processObject(object, options, path = "", output, nested) {
    if (nested && output) {
        output = { properties: output, fields: output }
    ...
    for (var key in object) {
        ...
        if (type === 'object') {
            let structureObject = {
                ...
                'type': {
                    'type': 'record',
                    'name': newPath,
                    ...processObject(value, options, newPath, output.properties[key])
                }
            }
            output.fields.push(structureObject)
            // output.properties[key] = processObject(value, options, path + key, output.properties[key])
            continue
        }
            ...
            let structureArray = {
                ...
                'type': {
                    ...
                    'items': {
                        'type': 'record',
                        'name': newPath,
                        ...processArray(value, options, newPath, output.properties[key])
                    }
                }
            }
            output.properties[key] = processArray(value, options, newPath, output.properties[key])
            continue
        }
        ...
        if (output.properties[key]) {
            ...
            // When multiple properties of differing types occur,
...
function processArray(array, options, path = "", output, nested) {
    var format
    var oneOf
    var type
    var value = []
    ...
    if (typeof output.items.oneOf !== 'undefined' || type === 'object') {
        for (var itemIndex = 0, itemLength = array.length; itemIndex < itemLength; itemIndex++) {
            ...
            if (itemType === 'object') {
                ...
                // }
                simpleArray = false
                arrayItem = processObject(value, options, path, oneOf ? {} : output.items.properties, true)
            } else if (itemType === 'array') {
                simpleArray = false
                arrayItem = processArray(value, options, path, oneOf ? {} : output.items.properties, true)
            } else {
                arrayItem = {}
...
function initialStructureAvro(properties) {
    let conteudo = {
        'type': 'record',

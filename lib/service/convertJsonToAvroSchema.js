...
function processObject(object, options, output, nested) {
    ...
    for (var key in object) {
        ...
        if (type === 'array') {
            output.fields.push({
                'name': key, 'type': type, 'items': processArray(value, options, key, output.fields[key]).items
            })
...
function processArray(array, options, parentKey, output, nested) {
    var format
    var oneOf
    var type
    ...
    if (nested && output) {
        ...
        output.items.name = output.items.name || parentKey
    ...
    if (typeof output.items.oneOf !== 'undefined' || type === 'record') {
        for (var itemIndex = 0, itemLength = array.length; itemIndex < itemLength; itemIndex++) {
            ...
            var arrayItem = {}
            if (itemType === 'record') {
                arrayItem.fields = processObject(value, options, oneOf ? {} : output.items.fields, true)
                console.log("")
            } else if (itemType === 'array') {
                arrayItem.items = processArray(value, options, "", oneOf ? {} : output.items.fields, true)
            } else {
                arrayItem = {}
                arrayItem.name = parentKey
